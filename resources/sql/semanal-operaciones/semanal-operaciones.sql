SELECT
   ...
   (
	    ...
	    FROM cobranzas.cobranza_agencia ca2
	    WHERE ca2.cobranza_id = c.id
	    LIMIT 1
	) AS agencia,
   0 AS grupo,
   em.rut,
...
LEFT JOIN LATERAL (
    SELECT
        ...
        em1.resolucion_id,
        em1.remesa_id
    FROM remesas.empleador em1
    WHERE em1.rut = c.rut_empleador
      AND em1.resolucion_id = c.resolucion::text
      AND em1.remesa_id = c.remesa_id
    ORDER BY em1.id ASC
    LIMIT 1
) em ON TRUE
-- ============================================================
-- CAUSA / ROL
-- ============================================================
LEFT JOIN cobranzas.cobranza_rit cr
   ON cr.cobranza_id = c.id
-- ============================================================
-- TRIBUNAL / JUZGADO
-- ============================================================
LEFT JOIN gui.tribunal tr
   ON tr.id = cr.tribunal_sp
-- ============================================================
-- MONTO RESOLUCIÓN
-- ============================================================
LEFT JOIN LATERAL (
